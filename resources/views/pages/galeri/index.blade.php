                            ...
                            <div class="container-fluid">
                              <nav class="navbar navbar-expand-md">
                                ...
                                <div class="collapse navbar-collapse mt-2 mt-md-0" id="navbarNav0">
                                  <p class="m-auto">
                                    <a class="btn btn-outline-primary m-1" href="{{route('galeri.create')}}">Tambah</a>
                        ...
                        <div class="row text-left">
                          <div class="col-12 col-sm-12 col-md-12 col-lg-12 m-sm-auto mr-md-auto ml-md-0">
                            <table id="dt4" class="table table-sm">
                              ...
                              <tbody>
                                @forelse($galeris as $key => $v)
                                <tr>
                                  <th scope="row">{{$key+1}}</th>
                                  <td class="text-center"><img height="120" src="{{asset($v->path.$v->filename)}}"></td>
                                  <td class="text-center">{{$v->caption}}</td>
                                  <td class="text-center">{{$v->tautan}}</td>
                                  <td class="text-right">
                                    <div class="dropdown">
                                      ...
                                      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a class="dropdown-item" href="{{ route('galeri.edit',$v->id) }}">Edit</a>
                                        <form id="delete-form-{{$v->id}}"
                                            method="post"
                                            action="{{route('galeri.destroy', $v->id) }}"
                                            style="display: none;">
                                            {{csrf_field()}}
                                            {{method_field('DELETE')}}
                                        </form>
